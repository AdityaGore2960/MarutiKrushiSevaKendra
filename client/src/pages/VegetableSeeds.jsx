...
const VegetableSeedsLocalizedContent = ({ page }) => (
  <div className="mt-16 bg-white rounded-2xl p-6 md:p-8 border border-gray-100 shadow-sm text-gray-600 space-y-6">
    {page.sections.map((section) => (
      <section key={section.heading}>
        ...
        {section.paragraphs?.map((paragraph) => (
          <p key={paragraph} className="text-sm leading-relaxed mb-2">{paragraph}</p>
        ))}
        {section.items && (
          <ul className="list-disc pl-5 text-sm space-y-1">
            {section.items.map((item) => (
              <li key={Array.isArray(item) ? item[0] : item}>
                {Array.isArray(item) ? <><strong>{item[0]}</strong> {item[1]}</> : item}
              </li>
            ))}
          </ul>
        )}
    ...
    <div className="pt-4 border-t border-gray-100">
      <p className="text-xs text-gray-400 italic">{page.disclaimer}</p>
    </div>
  </div>
);
...
const VegetableSeeds = () => {
  ...
  const page = t('vegetableSeedsPage');
  ...
  useEffect(() => {
    setLoading(true);
    setError('');
    getProducts({ categorySlug: 'vegetable-seeds' })
      ...
      <div className="max-w-6xl mx-auto px-4 py-10">
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-white rounded-2xl border border-gray-100 p-6">
            <p className="text-gray-500 text-sm font-medium">{t('vegetableSeedsPage.noProducts')}</p>
          </div>
        )}

        {/* SEO / Extra Content Section */}
        {language === 'mr' ? <VegetableSeedsLocalizedContent page={page} /> : <div className="mt-16 bg-white rounded-2xl p-6 md:p-8 border border-gray-100 shadow-sm text-gray-600 space-y-6">
          <section>
